...
fun dates_in_months (da: (int * int * int) list, m : int list) =
	...
	fun find_dates (m) =
	    if null m
	    then []
	    else dates_in_month(da, hd m) :: find_dates(tl m)

	fun append (lod : (int * int * int) list, nlod : (int * int * int) list) =
	    if null lod
	    then nlod
	    else hd lod :: append(tl lod, nlod)
			 
       fun iterate (lod : (int * int * int) list list, nlod : (int * int * int) list) =
	   if null lod
	   then nlod
	   else iterate (tl lod, append(hd lod, nlod))
		     		       
		       
    in
	iterate(find_dates(m), [])
    end
